...
function HintPage() {
  ...
  if (str.length > 0) {
    for (var j = 0; j < hints[idx].sounds.length; j++) {
      ...
      if (len >= str.length) {
        if (len > str.length) {// setHintAndHighlight(idx, j)
        } else {
          if (j + 1 == hints[idx].sounds.length) {
            // last sound
            hints[idx].hint = "";
            highlights[idx] = "";
          } else {// setHintAndHighlight(idx, j+1)
          }
        }
  ...
  } else if (str.length == 0) {// setHintAndHighlight(idx, 0)
  }
...
//# sourceMappingURL=hint.js.1d802c8b15ce20fac6d6.hot-update.js.map